...
query:
    ...
    | (simpleStatement SEMICOLON_SYMBOL)+ simpleStatement (SEMICOLON_SYMBOL EOF? | EOF)
;
...
simpleStatement:
    ...
    | {p.ServerVersion >= 80000}? importStatement
    ...
    // Data Directory
    | {p.ServerVersion >= 80000}? cloneStatement
    ...
    | showStatement
    | {p.ServerVersion >= 80000}? resourceGroupManagement
    | otherAdministrativeStatement

    // MySQL utilitity statements
    | utilityStatement
    | {p.ServerVersion >= 50604}? getDiagnostics
    | signalStatement
    | resignalStatement
;
...
alterStatement:
    ALTER_SYMBOL (
        ...
        | alterEvent
        | alterTablespace
        | {p.ServerVersion >= 80014}? alterUndoTablespace
        | alterLogfileGroup
        | alterServer
        // ALTER USER is part of the user management rule.
        | {p.ServerVersion >= 50713}? INSTANCE_SYMBOL ROTATE_SYMBOL textOrIdentifier MASTER_SYMBOL KEY_SYMBOL
    )
;
...
alterDatabase:
    DATABASE_SYMBOL schemaRef? (
        ...
        | {p.ServerVersion < 80000}? UPGRADE_SYMBOL DATA_SYMBOL DIRECTORY_SYMBOL NAME_SYMBOL
    )
;
...
alterTable:
    onlineOption? ({p.ServerVersion < 50700}? IGNORE_SYMBOL)? TABLE_SYMBOL tableRef alterTableActions?
;
...
standaloneAlterCommands:
    ...
    | alterPartition
    | {p.ServerVersion >= 80014}? (SECONDARY_LOAD_SYMBOL | SECONDARY_UNLOAD_SYMBOL)
;
...
alterPartition:
    ...
    | {p.ServerVersion >= 50704}? DISCARD_SYMBOL PARTITION_SYMBOL allOrPartitionNameList TABLESPACE_SYMBOL
    | {p.ServerVersion >= 50704}? IMPORT_SYMBOL PARTITION_SYMBOL allOrPartitionNameList TABLESPACE_SYMBOL
;
...
alterListItem:
    ...
    | DROP_SYMBOL (
        ...
        | FOREIGN_SYMBOL KEY_SYMBOL (
            ...
            {p.ServerVersion >= 50700}? columnInternalRef
            | {p.ServerVersion < 50700}? columnInternalRef?
        )
        | PRIMARY_SYMBOL KEY_SYMBOL
        | keyOrIndex indexRef
        | {p.ServerVersion >= 80017}? CHECK_SYMBOL identifier
        | {p.ServerVersion >= 80019}? CONSTRAINT_SYMBOL identifier
    )
    | DISABLE_SYMBOL KEYS_SYMBOL
    | ENABLE_SYMBOL KEYS_SYMBOL
    | ALTER_SYMBOL COLUMN_SYMBOL? columnInternalRef (
        SET_SYMBOL DEFAULT_SYMBOL (
            {p.ServerVersion >= 80014}? exprWithParentheses
            | signedLiteral
        )
        | DROP_SYMBOL DEFAULT_SYMBOL
    )
    | {p.ServerVersion >= 80000}? ALTER_SYMBOL INDEX_SYMBOL indexRef visibility
    | {p.ServerVersion >= 80017}? ALTER_SYMBOL CHECK_SYMBOL identifier constraintEnforcement
    | {p.ServerVersion >= 80019}? ALTER_SYMBOL CONSTRAINT_SYMBOL identifier constraintEnforcement
    | {p.ServerVersion >= 80000}? RENAME_SYMBOL COLUMN_SYMBOL columnInternalRef TO_SYMBOL identifier
    | RENAME_SYMBOL (TO_SYMBOL | AS_SYMBOL)? tableName
    | {p.ServerVersion >= 50700}? RENAME_SYMBOL keyOrIndex indexRef TO_SYMBOL indexName
    | CONVERT_SYMBOL TO_SYMBOL charset (
        {p.ServerVersion >= 80014}? DEFAULT_SYMBOL
        | charsetName
    ) collate?
    | FORCE_SYMBOL
    | ORDER_SYMBOL BY_SYMBOL alterOrderList
    | {p.ServerVersion >= 50708 && p.ServerVersion < 80000}? UPGRADE_SYMBOL PARTITIONING_SYMBOL
;
...
withValidation:
    {p.ServerVersion >= 50706}? (WITH_SYMBOL | WITHOUT_SYMBOL) VALIDATION_SYMBOL
;
...
alterTablespace:
    TABLESPACE_SYMBOL tablespaceRef (
        ...
        | RENAME_SYMBOL TO_SYMBOL identifier
        | {p.ServerVersion >= 80014}? alterTablespaceOptions
    )
;
...
createStatement:
    CREATE_SYMBOL (
        ...
        | createTablespace
        | createEvent
        | {p.ServerVersion >= 80000}? createRole
        | {p.ServerVersion >= 80011}? createSpatialReference
        | {p.ServerVersion >= 80014}? createUndoTablespace
    )
;
...
createDatabaseOption:
    defaultCharset
    | defaultCollation
    | {p.ServerVersion >= 80016}? defaultEncryption
;
...
createUdf:
    AGGREGATE_SYMBOL? FUNCTION_SYMBOL udfName RETURNS_SYMBOL typeName = (
        STRING_SYMBOL
        | INT_SYMBOL
        | REAL_SYMBOL
        | DECIMAL_SYMBOL
...
createIndex:
    onlineOption? (
        UNIQUE_SYMBOL? typeName = INDEX_SYMBOL (
            {p.ServerVersion >= 80014}? indexName indexTypeClause?
            | indexNameAndType?
        ) createIndexTarget indexOption*
        | typeName = FULLTEXT_SYMBOL INDEX_SYMBOL indexName createIndexTarget fulltextIndexOption*
        | typeName = SPATIAL_SYMBOL INDEX_SYMBOL indexName createIndexTarget spatialIndexOption*
    ) indexLockAndAlgorithm?
;
...
tsDataFileName:
    {p.ServerVersion >= 80014}? (ADD_SYMBOL tsDataFile)?
...
tablespaceOption:
    ...
    | tsOptionWait
    | tsOptionComment
    | {p.ServerVersion >= 50707}? tsOptionFileblockSize
    | {p.ServerVersion >= 80014}? tsOptionEncryption
;
...
triggerFollowsPrecedesClause:
    {p.ServerVersion >= 50700}? ordering = (FOLLOWS_SYMBOL | PRECEDES_SYMBOL) textOrIdentifier // not a trigger reference!
;
...
dropStatement:
    DROP_SYMBOL (
        ...
        | dropTrigger
        | dropView
        | {p.ServerVersion >= 80000}? dropRole
        | {p.ServerVersion >= 80011}? dropSpatialReference
        | {p.ServerVersion >= 80014}? dropUndoTablespace
    )
;
...
dropIndex:
    typeName = INDEX_SYMBOL onlineOption? indexRef ON_SYMBOL tableRef indexLockAndAlgorithm?
;
...
dropTable:
    TEMPORARY_SYMBOL? typeName = (TABLE_SYMBOL | TABLES_SYMBOL) ifExists? tableRefList (
        RESTRICT_SYMBOL
        | CASCADE_SYMBOL
    )?
;
...
deleteStatement:
    ({p.ServerVersion >= 80000}? withClause)? DELETE_SYMBOL deleteStatementOption* (
        FROM_SYMBOL (
            tableAliasRefList USING_SYMBOL tableReferenceList whereClause?       // Multi table variant 1.
            | tableRef ({p.ServerVersion >= 80017}? tableAlias)? partitionDelete?
...
partitionDelete:
    {p.ServerVersion >= 50602}? PARTITION_SYMBOL OPEN_PAR_SYMBOL identifierList CLOSE_PAR_SYMBOL
;
...
doStatement:
    DO_SYMBOL (
        {p.ServerVersion < 50709}? exprList
        | {p.ServerVersion >= 50709}? selectItemList
    )
;
...
insertStatement:
    INSERT_SYMBOL insertLockOption? IGNORE_SYMBOL? INTO_SYMBOL? tableRef usePartition? (
        insertFromConstructor ({ p.ServerVersion >= 80018}? valuesReference)?
        | SET_SYMBOL updateList ({ p.ServerVersion >= 80018}? valuesReference)?
...
values:
    exprOrDefault (COMMA_SYMBOL exprOrDefault)*
;
...
queryPrimary:
    querySpecification
    | {p.ServerVersion >= 80019}? tableValueConstructor
    | {p.ServerVersion >= 80019}? explicitTable
;

querySpecification:
    SELECT_SYMBOL selectOption* selectItemList intoClause? fromClause? whereClause? groupByClause? havingClause? (
        {p.ServerVersion >= 80000}? windowClause
    )?
;
...
olapOption:
    ...
    | {p.ServerVersion < 80000}? WITH_SYMBOL CUBE_SYMBOL
;
...
selectOption:
    ...
    | {p.ServerVersion < 80000}? SQL_CACHE_SYMBOL
    | {p.ServerVersion >= 50704 && p.ServerVersion < 50708}? MAX_STATEMENT_TIME_SYMBOL EQUAL_OPERATOR real_ulong_number
;
...
lockingClause:
    FOR_SYMBOL lockStrengh ({p.ServerVersion >= 80000}? OF_SYMBOL tableAliasRefList)? (
        {p.ServerVersion >= 80000}? lockedRowAction
    )?
...
lockStrengh:
    UPDATE_SYMBOL
    | {p.ServerVersion >= 80000}? SHARE_SYMBOL
;
...
tableReference: ( // Note: we have also a tableRef rule for identifiers that reference a table anywhere.
        tableFactor
        | OPEN_CURLY_SYMBOL ({p.ServerVersion < 80017}? identifier | OJ_SYMBOL) escapedTableReference CLOSE_CURLY_SYMBOL // ODBC syntax
    ) joinedTable*
;
...
innerJoinType:
    typeName = (INNER_SYMBOL | CROSS_SYMBOL)? JOIN_SYMBOL
    | typeName = STRAIGHT_JOIN_SYMBOL
;

outerJoinType:
    typeName = (LEFT_SYMBOL | RIGHT_SYMBOL) OUTER_SYMBOL? JOIN_SYMBOL
;
...
tableFactor:
    ...
    | {p.ServerVersion >= 80004}? tableFunction
;
...
derivedTable:
    subquery tableAlias? ({p.ServerVersion >= 80000}? columnInternalRefList)?
    | {p.ServerVersion >= 80014}? LATERAL_SYMBOL subquery tableAlias? columnInternalRefList?
;
...
jtColumn:
    ...
    | identifier dataType ({p.ServerVersion >= 80014}? collate)? EXISTS_SYMBOL? PATH_SYMBOL textStringLiteral onEmptyOrError?
...
tableAlias:
    (AS_SYMBOL | {p.ServerVersion < 80017}? EQUAL_OPERATOR)? identifier
;
...
updateStatement:
    ({p.ServerVersion >= 80000}? withClause)? UPDATE_SYMBOL LOW_PRIORITY_SYMBOL? IGNORE_SYMBOL? tableReferenceList SET_SYMBOL
...
transactionCharacteristic:
    ...
    | {p.ServerVersion >= 50605}? READ_SYMBOL (WRITE_SYMBOL | ONLY_SYMBOL)
;
...
lockStatement:
    ...
    | {p.ServerVersion >= 80000}? LOCK_SYMBOL INSTANCE_SYMBOL FOR_SYMBOL BACKUP_SYMBOL
    | UNLOCK_SYMBOL (
        TABLES_SYMBOL
        | TABLE_SYMBOL
        | {p.ServerVersion >= 80000}? INSTANCE_SYMBOL
    )
;
...
xaConvert:
    {p.ServerVersion >= 50704}? (CONVERT_SYMBOL XID_SYMBOL)?
    | /* empty */
;
...
replicationStatement:
    ...
    | {p.ServerVersion > 80000}? RESET_SYMBOL PERSIST_SYMBOL (ifExists identifier)?
    | slave
    | {p.ServerVersion >= 50700}? changeReplication
    | replicationLoad
    | {p.ServerVersion > 50706}? groupReplication
;

resetOption:
    option = MASTER_SYMBOL masterResetOptions?
    | {p.ServerVersion < 80000}? option = QUERY_SYMBOL CACHE_SYMBOL
    | option = SLAVE_SYMBOL ALL_SYMBOL? channel?
;

masterResetOptions:
    {p.ServerVersion >= 80000}? TO_SYMBOL (
        {p.ServerVersion < 80017}? real_ulong_number
        | {p.ServerVersion >= 80017}? real_ulonglong_number
    )
;
...
slaveUntilOptions:
    (
        masterFileDef
        | {p.ServerVersion >= 50606}? (
            SQL_BEFORE_GTIDS_SYMBOL
            | SQL_AFTER_GTIDS_SYMBOL
        ) EQUAL_OPERATOR textString
        | {p.ServerVersion >= 50606}? SQL_AFTER_MTS_GAPS_SYMBOL
    ) (COMMA_SYMBOL masterFileDef)*
;

slaveConnectionOptions:
    {p.ServerVersion >= 50604}? (USER_SYMBOL EQUAL_OPERATOR textString)? (
...
preparedStatement:
    typeName = PREPARE_SYMBOL identifier FROM_SYMBOL (textLiteral | userVariable)
    | executeStatement
    | typeName = (DEALLOCATE_SYMBOL | DROP_SYMBOL) PREPARE_SYMBOL identifier
;
...
cloneStatement:
    CLONE_SYMBOL (
        ...
        | {p.ServerVersion >= 80014}? INSTANCE_SYMBOL FROM_SYMBOL user COLON_SYMBOL ulong_number IDENTIFIED_SYMBOL BY_SYMBOL
...
accountManagementStatement:
    {p.ServerVersion >= 50606}? alterUser
    | createUser
    | dropUser
    | grant
    | renameUser
    | revoke
    | {p.ServerVersion >= 80000}? setRole
;

alterUser:
    ALTER_SYMBOL USER_SYMBOL ({p.ServerVersion >= 50706}? ifExists)? alterUserTail
;

alterUserTail:
    ({p.ServerVersion < 80014}? createUserList | {p.ServerVersion >= 80014}? alterUserList) createUserTail
    | {p.ServerVersion >= 50706}? user IDENTIFIED_SYMBOL BY_SYMBOL textString (
        {p.ServerVersion >= 80014}? replacePassword
    )? ({p.ServerVersion >= 80014}? retainCurrentPassword)?
    | {p.ServerVersion >= 80014}? user discardOldPassword
    | {p.ServerVersion >= 80000}? user DEFAULT_SYMBOL ROLE_SYMBOL (
        ALL_SYMBOL
        | NONE_SYMBOL
        | roleList
    )
    | {p.ServerVersion >= 80018}? user IDENTIFIED_SYMBOL (WITH_SYMBOL textOrIdentifier)? BY_SYMBOL RANDOM_SYMBOL
...
createUser:
    CREATE_SYMBOL USER_SYMBOL ({p.ServerVersion >= 50706}? ifNotExists | /* empty */) createUserList defaultRoleClause
        createUserTail
;

createUserTail:
    {p.ServerVersion >= 50706}? requireClause? connectOptions? accountLockPasswordExpireOptions*
    | /* empty */
;

defaultRoleClause:
    {p.ServerVersion >= 80000}? (DEFAULT_SYMBOL ROLE_SYMBOL roleList)?
    | /* empty */
;
...
accountLockPasswordExpireOptions:
    ...
    | PASSWORD_SYMBOL (
        ...
        | {p.ServerVersion >= 80014}? REQUIRE_SYMBOL CURRENT_SYMBOL (
            DEFAULT_SYMBOL
            | OPTIONAL_SYMBOL
        )?
    )
...
dropUser:
    DROP_SYMBOL USER_SYMBOL ({p.ServerVersion >= 50706}? ifExists)? userList
;

grant:
    GRANT_SYMBOL (
        {p.ServerVersion >= 80000}? roleOrPrivilegesList TO_SYMBOL userList (
...
grantTargetList:
    {p.ServerVersion < 80011}? createUserList
    | {p.ServerVersion >= 80011}? userList
;

grantOptions:
    {p.ServerVersion < 80011}? WITH_SYMBOL grantOption+
    | {p.ServerVersion >= 80011}? WITH_SYMBOL GRANT_SYMBOL OPTION_SYMBOL
;
...
versionedRequireClause:
    {p.ServerVersion < 80011}? requireClause
;
...
revoke:
    REVOKE_SYMBOL (
        {p.ServerVersion >= 80000}? roleOrPrivilegesList FROM_SYMBOL userList
        | roleOrPrivilegesList onTypeTo FROM_SYMBOL userList
        | ALL_SYMBOL PRIVILEGES_SYMBOL? (
            {p.ServerVersion >= 80000}? ON_SYMBOL aclType? grantIdentifier
...
roleOrPrivilege:
    ...
    | {p.ServerVersion > 80000}? (CREATE_SYMBOL | DROP_SYMBOL) ROLE_SYMBOL
;
...
grantIdentifier:
    ...
    | tableRef
    | {p.ServerVersion >= 80017}? schemaRef DOT_SYMBOL tableRef
;
...
tableAdministrationStatement:
    typeName = ANALYZE_SYMBOL noWriteToBinLog? TABLE_SYMBOL tableRefList (
        {p.ServerVersion >= 80000}? histogram
    )?
    | typeName = CHECK_SYMBOL TABLE_SYMBOL tableRefList checkOption*
    | typeName = CHECKSUM_SYMBOL TABLE_SYMBOL tableRefList (
        QUICK_SYMBOL
        | EXTENDED_SYMBOL
    )?
    | typeName = OPTIMIZE_SYMBOL noWriteToBinLog? TABLE_SYMBOL tableRefList
    | typeName = REPAIR_SYMBOL noWriteToBinLog? TABLE_SYMBOL tableRefList repairType*
;
...
installUninstallStatment:
    ...
    actionName = INSTALL_SYMBOL typeName = PLUGIN_SYMBOL identifier SONAME_SYMBOL textStringLiteral
    | actionName = INSTALL_SYMBOL typeName = COMPONENT_SYMBOL textStringLiteralList
    | actionName = UNINSTALL_SYMBOL typeName = PLUGIN_SYMBOL pluginRef
    | actionName = UNINSTALL_SYMBOL typeName = COMPONENT_SYMBOL componentRef (
...
startOptionValueList:
    ...
    | PASSWORD_SYMBOL (FOR_SYMBOL user)? equal (
        ...
        | textString replacePassword? retainCurrentPassword?
        | {p.ServerVersion < 50706}? OLD_PASSWORD_SYMBOL OPEN_PAR_SYMBOL textString CLOSE_PAR_SYMBOL
        | {p.ServerVersion < 80014}? PASSWORD_SYMBOL OPEN_PAR_SYMBOL textString CLOSE_PAR_SYMBOL
    )
    | {p.ServerVersion >= 80018}? PASSWORD_SYMBOL (FOR_SYMBOL user)? TO_SYMBOL RANDOM_SYMBOL replacePassword? retainCurrentPassword?
;
...
optionValueNoOptionType:
    ...
    | NAMES_SYMBOL (
        equal expr
        | charsetName collate?
        | {p.ServerVersion >= 80011}? DEFAULT_SYMBOL
    )
;
...
setExprOrDefault:
    ...
    | {p.ServerVersion >= 80000}? (ROW_SYMBOL | SYSTEM_SYMBOL)
;
...
showStatement:
    SHOW_SYMBOL (
        {p.ServerVersion < 50700}? value = AUTHORS_SYMBOL
        ...
        | ({p.ServerVersion >= 80000}? EXTENDED_SYMBOL)? value = (
            INDEX_SYMBOL
            | INDEXES_SYMBOL
            | KEYS_SYMBOL
        ...
        | {p.ServerVersion < 50700}? value = CONTRIBUTORS_SYMBOL
        ...
        | value = CREATE_SYMBOL (
            ...
            | {p.ServerVersion >= 50704}? object = USER_SYMBOL user
        )
...
showCommandType:
    FULL_SYMBOL
    | {p.ServerVersion >= 80000}? EXTENDED_SYMBOL FULL_SYMBOL?
;

nonBlocking:
    {p.ServerVersion >= 50700 && p.ServerVersion < 50706}? NONBLOCKING_SYMBOL?
    | /* empty */
;
...
otherAdministrativeStatement:
    typeName = BINLOG_SYMBOL textLiteral
    | typeName = CACHE_SYMBOL INDEX_SYMBOL keyCacheListOrParts IN_SYMBOL (
        identifier
        | DEFAULT_SYMBOL
    )
    | typeName = FLUSH_SYMBOL noWriteToBinLog? (
        flushTables
        | flushOption (COMMA_SYMBOL flushOption)*
    )
    | typeName = KILL_SYMBOL (CONNECTION_SYMBOL | QUERY_SYMBOL)? expr
    | typeName = LOAD_SYMBOL INDEX_SYMBOL INTO_SYMBOL CACHE_SYMBOL preloadTail
    | {p.ServerVersion >= 50709}? typeName = SHUTDOWN_SYMBOL
;
...
flushOption:
    ...
    | {p.ServerVersion < 80000}? option = QUERY_SYMBOL CACHE_SYMBOL
    | {p.ServerVersion >= 50706}? option = OPTIMIZER_COSTS_SYMBOL
;
...
flushTablesOptions:
    {p.ServerVersion >= 50606}? FOR_SYMBOL EXPORT_SYMBOL
    | WITH_SYMBOL READ_SYMBOL LOCK_SYMBOL
;
...
utilityStatement:
    ...
    | helpCommand
    | useCommand
    | {p.ServerVersion >= 80011}? restartServer
;
...
explainStatement:
    (EXPLAIN_SYMBOL | DESCRIBE_SYMBOL | DESC_SYMBOL) (
        {p.ServerVersion < 80000}? EXTENDED_SYMBOL
        | {p.ServerVersion < 80000}? PARTITIONS_SYMBOL
        | {p.ServerVersion >= 50605}? FORMAT_SYMBOL EQUAL_OPERATOR textOrIdentifier
        | {p.ServerVersion >= 80018}? ANALYZE_SYMBOL
        | {p.ServerVersion >= 80019}? ANALYZE_SYMBOL FORMAT_SYMBOL EQUAL_OPERATOR textOrIdentifier
...
explainableStatement:
    selectStatement
    | {p.ServerVersion >= 50603}? (
        deleteStatement
        | insertStatement
        | replaceStatement
        | updateStatement
    )
    | {p.ServerVersion >= 50700}? FOR_SYMBOL CONNECTION_SYMBOL real_ulong_number
;
...
expr:
    boolPri (IS_SYMBOL notRule? typeName = (TRUE_SYMBOL | FALSE_SYMBOL | UNKNOWN_SYMBOL))? # exprIs
...
predicate:
    bitExpr (
        ...
        | {p.ServerVersion >= 80017}? MEMBER_SYMBOL OF_SYMBOL? simpleExprWithParentheses
...
simpleExpr:
    ...
    | {p.ServerVersion >= 80000}? groupingOperation                                                        # simpleExprGroupingOperation
    | {p.ServerVersion >= 80000}? windowFunctionCall                                                       # simpleExprWindowingFunction
...
arrayCast:
    {p.ServerVersion >= 80017}? ARRAY_SYMBOL
;

jsonOperator:
    {p.ServerVersion >= 50708}? JSON_SEPARATOR_SYMBOL textStringLiteral
    | {p.ServerVersion >= 50713}? JSON_UNQUOTED_SEPARATOR_SYMBOL textStringLiteral
;

sumExpr:
    name = AVG_SYMBOL OPEN_PAR_SYMBOL DISTINCT_SYMBOL? inSumExpr CLOSE_PAR_SYMBOL (
        {p.ServerVersion >= 80000}? windowingClause
    )?
    | name = (BIT_AND_SYMBOL | BIT_OR_SYMBOL | BIT_XOR_SYMBOL) OPEN_PAR_SYMBOL inSumExpr CLOSE_PAR_SYMBOL (
        {p.ServerVersion >= 80000}? windowingClause
    )?
    | {p.ServerVersion >= 80000}? jsonFunction
    ...
    ) CLOSE_PAR_SYMBOL ({p.ServerVersion >= 80000}? windowingClause)?
...
runtimeFunctionCall:
    ...
    | name = (TIMESTAMPADD_SYMBOL | TIMESTAMPDIFF_SYMBOL) OPEN_PAR_SYMBOL intervalTimeStamp COMMA_SYMBOL expr COMMA_SYMBOL expr
        CLOSE_PAR_SYMBOL
    ...
    | {p.ServerVersion < 50607}? name = OLD_PASSWORD_SYMBOL OPEN_PAR_SYMBOL textLiteral CLOSE_PAR_SYMBOL
    | {p.ServerVersion < 80011}? name = PASSWORD_SYMBOL exprWithParentheses
    ...
    | name = WEIGHT_STRING_SYMBOL OPEN_PAR_SYMBOL expr (
        (AS_SYMBOL CHAR_SYMBOL wsNumCodepoints)? (
            {p.ServerVersion < 80000}? weightStringLevels
        )?
...
geometryFunction:
    {p.ServerVersion < 50706}? name = CONTAINS_SYMBOL OPEN_PAR_SYMBOL expr COMMA_SYMBOL expr CLOSE_PAR_SYMBOL
...
fractionalPrecision:
    {p.ServerVersion >= 50604}? INT_NUMBER
;
...
userVariable:
    AT_SIGN_SYMBOL identifier
    | AT_TEXT_SUFFIX
;

systemVariable:
    AT_AT_SIGN_SYMBOL varIdentType? identifier dotIdentifier?
;

internalVariableName: (
        // Check in semantic phase that the first id is not global/local/session/default.
        {p.ServerVersion < 80017}? identifier dotIdentifier?
        | {p.ServerVersion >= 80017}? lValueIdentifier dotIdentifier?
    )
...
castType:
    ...
    | {p.ServerVersion >= 50708}? JSON_SYMBOL
    | {p.ServerVersion >= 80017}? realType
    | {p.ServerVersion >= 80017}? FLOAT_SYMBOL standardFloatOptions?
;
...
channel:
    {p.ServerVersion >= 50706}? FOR_SYMBOL CHANNEL_SYMBOL textStringNoLinebreak
;
...
getDiagnostics:
    GET_SYMBOL (CURRENT_SYMBOL | {p.ServerVersion >= 50700}? STACKED_SYMBOL)? DIAGNOSTICS_SYMBOL (
...
checkOrReferences:
    {p.ServerVersion < 80016}? checkConstraint
    | references
;
...
tableConstraintDef:
    typeName = (KEY_SYMBOL | INDEX_SYMBOL) indexNameAndType? keyListVariants indexOption*
    | typeName = FULLTEXT_SYMBOL keyOrIndex? indexName? keyListVariants fulltextIndexOption*
    | typeName = SPATIAL_SYMBOL keyOrIndex? indexName? keyListVariants spatialIndexOption*
    | constraintName? (
        (typeName = PRIMARY_SYMBOL KEY_SYMBOL | typeName = UNIQUE_SYMBOL keyOrIndex?) indexNameAndType? keyListVariants indexOption*
        | typeName = FOREIGN_SYMBOL KEY_SYMBOL indexName? keyList references
        | checkConstraint ({p.ServerVersion >= 80017}? constraintEnforcement)?
    )
;
...
fieldDefinition:
    dataType (
        columnAttribute*
        | {p.ServerVersion >= 50707}? collate? (GENERATED_SYMBOL ALWAYS_SYMBOL)? AS_SYMBOL exprWithParentheses (
            VIRTUAL_SYMBOL
            | STORED_SYMBOL
        )? (
            {p.ServerVersion < 80000}? gcolAttribute*
            | {p.ServerVersion >= 80000}? columnAttribute* // Beginning with 8.0 the full attribute set is supported.
        )
    )
;
...
columnAttribute:
    ...
    | {p.ServerVersion >= 80014}? NOT_SYMBOL SECONDARY_SYMBOL
    | value = DEFAULT_SYMBOL (
        signedLiteral
        | NOW_SYMBOL timeFunctionParameters?
        | {p.ServerVersion >= 80013}? exprWithParentheses
    )
    ...
    | {p.ServerVersion >= 80000}? value = SRID_SYMBOL real_ulonglong_number
    | {p.ServerVersion >= 80017}? constraintName? checkConstraint constraintEnforcement?
;
...
deleteOption:
    ...
    | SET_SYMBOL (nullLiteral | DEFAULT_SYMBOL)
...
keyListVariants:
    {p.ServerVersion >= 80013}? keyListWithExpression
    | {p.ServerVersion < 80013}? keyList
;
...
commonIndexOption:
    ...
    | COMMENT_SYMBOL textLiteral
    | {p.ServerVersion >= 80000}? visibility
;
...
dataType: // type in sql_yacc.yy
    ...
    | (typeName = REAL_SYMBOL | typeName = DOUBLE_SYMBOL PRECISION_SYMBOL?) precision? fieldOptions?
    | typeName = (FLOAT_SYMBOL | DECIMAL_SYMBOL | NUMERIC_SYMBOL | FIXED_SYMBOL) floatOptions? fieldOptions?
    | typeName = BIT_SYMBOL fieldLength?
    | typeName = (BOOL_SYMBOL | BOOLEAN_SYMBOL)
    | typeName = CHAR_SYMBOL fieldLength? charsetWithOptBinary?
    | nchar fieldLength? BINARY_SYMBOL?
    | typeName = BINARY_SYMBOL fieldLength?
    | (typeName = CHAR_SYMBOL VARYING_SYMBOL | typeName = VARCHAR_SYMBOL) fieldLength charsetWithOptBinary?
    | (
        typeName = NATIONAL_SYMBOL VARCHAR_SYMBOL
        | typeName = NVARCHAR_SYMBOL
        | typeName = NCHAR_SYMBOL VARCHAR_SYMBOL
        | typeName = NATIONAL_SYMBOL CHAR_SYMBOL VARYING_SYMBOL
        | typeName = NCHAR_SYMBOL VARYING_SYMBOL
    ) fieldLength BINARY_SYMBOL?
    | typeName = VARBINARY_SYMBOL fieldLength
    | typeName = YEAR_SYMBOL fieldLength? fieldOptions?
    | typeName = DATE_SYMBOL
    | typeName = TIME_SYMBOL typeDatetimePrecision?
    | typeName = TIMESTAMP_SYMBOL typeDatetimePrecision?
    | typeName = DATETIME_SYMBOL typeDatetimePrecision?
    | typeName = TINYBLOB_SYMBOL
    | typeName = BLOB_SYMBOL fieldLength?
    | typeName = (MEDIUMBLOB_SYMBOL | LONGBLOB_SYMBOL)
    | typeName = LONG_SYMBOL VARBINARY_SYMBOL
    | typeName = LONG_SYMBOL (CHAR_SYMBOL VARYING_SYMBOL | VARCHAR_SYMBOL)? charsetWithOptBinary?
    | typeName = TINYTEXT_SYMBOL charsetWithOptBinary?
    | typeName = TEXT_SYMBOL fieldLength? charsetWithOptBinary?
    | typeName = MEDIUMTEXT_SYMBOL charsetWithOptBinary?
    | typeName = LONGTEXT_SYMBOL charsetWithOptBinary?
    | typeName = ENUM_SYMBOL stringList charsetWithOptBinary?
    | typeName = SET_SYMBOL stringList charsetWithOptBinary?
    | typeName = SERIAL_SYMBOL
    | {p.ServerVersion >= 50708}? typeName = JSON_SYMBOL
    | typeName = (
        GEOMETRY_SYMBOL
...
nchar:
    typeName = NCHAR_SYMBOL
    | typeName = NATIONAL_SYMBOL CHAR_SYMBOL
;

realType:
    typeName = REAL_SYMBOL
    | typeName = DOUBLE_SYMBOL PRECISION_SYMBOL?
;
...
collationName:
    textOrIdentifier
    | {p.ServerVersion < 80011}? DEFAULT_SYMBOL
    | {p.ServerVersion >= 80018}? BINARY_SYMBOL
;
...
createTableOption: // In the order as they appear in the server grammar.
    ...
    | {p.ServerVersion >= 80014}? option = SECONDARY_ENGINE_SYMBOL equal? (
        NULL_SYMBOL
        | textOrIdentifier
    )
    ...
    | {p.ServerVersion >= 50708}? option = COMPRESSION_SYMBOL EQUAL_OPERATOR? textString
    | {p.ServerVersion >= 50711}? option = ENCRYPTION_SYMBOL EQUAL_OPERATOR? textString
    ...
    | option = ROW_FORMAT_SYMBOL EQUAL_OPERATOR? format = (
        ...
        | REDUNDANT_SYMBOL
        | COMPACT_SYMBOL
        | TOKUDB_DEFAULT_SYMBOL
        | TOKUDB_FAST_SYMBOL
        | TOKUDB_SMALL_SYMBOL
        | TOKUDB_ZLIB_SYMBOL
        | TOKUDB_QUICKLZ_SYMBOL
        | TOKUDB_LZMA_SYMBOL
        | TOKUDB_SNAPPY_SYMBOL
        | TOKUDB_UNCOMPRESSED_SYMBOL
    )
    ...
    | option = TABLESPACE_SYMBOL (
        {p.ServerVersion >= 50707}? EQUAL_OPERATOR?
        | /* empty */
    ) identifier
...
partitionKeyAlgorithm: // Actually only 1 and 2 are allowed. Needs a semantic check.
    {p.ServerVersion >= 50700}? ALGORITHM_SYMBOL EQUAL_OPERATOR real_ulong_number
;
...
procedureParameter:
    typeName = (IN_SYMBOL | OUT_SYMBOL | INOUT_SYMBOL)? functionParameter
;
...
createUserEntry: // create_user in sql_yacc.yy
    user (
        IDENTIFIED_SYMBOL (
            BY_SYMBOL ({p.ServerVersion < 80011}? PASSWORD_SYMBOL)? textString
            | WITH_SYMBOL textOrIdentifier (
                AS_SYMBOL textStringHash
                | {p.ServerVersion >= 50706}? BY_SYMBOL textString
            )?
            | {p.ServerVersion >= 80018}? (WITH_SYMBOL textOrIdentifier)? BY_SYMBOL RANDOM_SYMBOL PASSWORD_SYMBOL
        )
    )?
;
...
usePartition:
    {p.ServerVersion >= 50602}? PARTITION_SYMBOL identifierListWithParentheses
;
...
columnName:
    ...
    {p.ServerVersion >= 80000}? identifier
    | {p.ServerVersion < 80000}? fieldIdentifier
;
...
pureIdentifier:
    ...
    | {p.isSqlModeActive(AnsiQuotes)}? DOUBLE_QUOTED_TEXT
;
...
simpleIdentifier: // simple_ident + simple_ident_q
    ...
    | {p.ServerVersion < 80000}? dotIdentifier dotIdentifier
;
...
real_ulonglong_number:
    INT_NUMBER
    | {p.ServerVersion >= 80017}? HEX_NUMBER
    | ULONGLONG_NUMBER
    | LONG_NUMBER
;
...
textStringLiteral:
    ...
    | {!p.isSqlModeActive(AnsiQuotes)}? value = DOUBLE_QUOTED_TEXT
;
...
identifierKeyword:
    {p.ServerVersion < 80017}? (
        labelKeyword
        | roleOrIdentifierKeyword
        | EXECUTE_SYMBOL
        | {p.ServerVersion >= 50709}? SHUTDOWN_SYMBOL // Previously allowed as SP label as well.
        | {p.ServerVersion >= 80011}? RESTART_SYMBOL
    )
    | (
...
roleKeyword:
    {p.ServerVersion < 80017}? (roleOrLabelKeyword | roleOrIdentifierKeyword)
    | (
        identifierKeywordsUnambiguous
        | identifierKeywordsAmbiguous2Labels
        | identifierKeywordsAmbiguous4SystemVariables
...
roleOrIdentifierKeyword:
    ...
    | {p.ServerVersion >= 50709}? SHUTDOWN_SYMBOL
    | {p.ServerVersion >= 80000}? IMPORT_SYMBOL
;
...
roleOrLabelKeyword:
    ...
    | {p.ServerVersion >= 80014}? ADMIN_SYMBOL
;
